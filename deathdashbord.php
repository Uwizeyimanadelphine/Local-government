...
$sql = "SELECT * FROM deathcertificate";
$result = mysqli_query($conn,$sql);
if (mysqli_num_rows($result) > 0)  {
// output data of each row
while($row = $result->fetch_assoc()) {
    echo "<tr><td>" . $row["fname"]. "</td><td>" . $row["lname"] . "</td><td>" . $row["fathername"]. "</td><td>" 
    . $row["mothername"]. "</td><td>" . $row["status"]. "</td><td>"  . $row["sex"]. "</td><td>" . $row["dateofbirth"] . 
    "</td><td>" . $row["dateofdeath"]. "</td><td>" . $row["province"]. "</td><td>" . $row["district"]. "</td><td>". $row["sector"]. "</td><td>" . $row["village"].
     "</td><td>" . $row["cell"]. "</td>"  
    ?>
	<td><a href="viewdeath.php?id=<?php echo $row['d_id'];?>"> view </a></td>
		<td><a href="productdetails.php?id=<?php echo $row['d_id'];?>"> update </a></td>
		<td><a href="deletedeath.php?id=<?php echo $row['d_id'];?>"> delete </a></td>
	<?php
	"<br/></tr>";
}	
}
